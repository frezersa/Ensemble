...
cat(paste("2 - ",forecast_directory <- args[2]),"\n") #forecast directory
# script_directory <- "V:\\LW_Ens\\A_MS\\Repo\\scripts"
# forecast_directory <- "V:\\LW_Ens\\HydForecastStorage\\forecast_20161027"
...
getbiasedmembers <- function(member, forecast_directory){
  ...
  #get the forecast
  forecast <- lapply(1:length(hindcast$stations), getmetensembleforecast, file_paths=file_paths)
  names(forecast) <- ReadSplCsvWheader(file_paths[1])$stations
  
    
  #apply bias corrections to the forecast
  newforecast <- lapply(1:length(hindcast$stations), getbiasedforecasts, hindcast, forecast)
...
inflowplot <- function(member.plot){
  models <- sort(as.character(unique(member.plot$xConfig)))
  models <- models[models != 'All']
  ...
  p <- ggplot(data=member.plot, aes_string(x="xDay",y="x50",ymin="x5",ymax="x95")) +
    ...
    #modelled hindcast 1
    geom_line(data=member.plot[member.plot$xConfig==models[1],],aes(col="Mod1")) +
    
    #modelled hindcast 2
    geom_line(data=member.plot[member.plot$xConfig==models[2],],aes(col="Mod2")) +
...
if(length(membernames) > 1){
  m=1
  for(m in 1:length(res_names)){
